...
def process_all_sheets(stock_data, sentiment_data):
    ...
    combined_data = [merge_data(stock_df, sentiment_data, company) for company, stock_df in stock_data.items()]
...
def calculate_weighted_sentiment(sentiment_data, dowjones_data):
    ...
    sentiment_data['weighted_sentiment'] = sentiment_data['sentiment_score'] * sentiment_data['size']
    ...
    return pd.merge(dowjones_data, daily_sentiment[['date', 'daily_weighted_sentiment']], on='date', how='left')
...
if __name__ == "__main__":
    ...
    STOCK_SENTIMENT_VARIATION_OUTPUT = "data/processed_stock_sentiment_data_with_variation.xlsx"
    ...
    process_combined_data(DOWJONES_OUTPUT_FILE, STOCK_SENTIMENT_OUTPUT, DOWJONES_SENTIMENT_OUTPUT)
