...
def comments_analysis(all_comments):
    ...
    result = result.sort_values(by="total_comments", ascending=False).reset_index(drop=True)
...
def sentiment_analysis(all_comments):
    ...
    for i in tqdm(range(0, len(comments), batch_size), desc="Processing comments in batches"):
...
def get_sentiment_score(all_sentiments):
    ...
    for (stock, date), group in grouped:
        positive_weighted = group.loc[group['sentiment'] == 'positive', 'weighted_influence'].sum()
        negative_weighted = group.loc[group['sentiment'] == 'negative', 'weighted_influence'].sum()

        if (positive_weighted + negative_weighted) > 0:
            sentiment_score = (positive_weighted - negative_weighted) / (positive_weighted + negative_weighted)
        else:
            sentiment_score = 0
