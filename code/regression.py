...
def prepare_data_for_arima_var(file_path):
    ...
    df = df[df['daily_weighted_sentiment'].notna() & (df['daily_weighted_sentiment'] != 0)]
    ...
    for lag in range(1, 4):
        df[f'daily_weighted_sentiment_lag{lag}'] = df['daily_weighted_sentiment'].shift(lag)
    df['daily_weighted_sentiment_lag3_diff'] = df['daily_weighted_sentiment_lag3'].diff()
    df = df.dropna(subset=['return_diff', 'volume_diff', 'sentiment_diff',"log_return",
                           'daily_weighted_sentiment_lag1', 'daily_weighted_sentiment_lag2', 'daily_weighted_sentiment_lag3','daily_weighted_sentiment_lag3_diff'])
    return df
...
def forecast_var(var_model, data, title, filename, steps, output_dir):
    ...
    ax1.plot(range(steps), forecast_df.iloc[:, 0], label=f'{data.columns[0]} Forecast', color='blue')
    ax1.set_ylabel(data.columns[0], color='blue')
    ax1.tick_params(axis='y', labelcolor='blue')
    ax2 = ax1.twinx()
    ax2.plot(range(steps), forecast_df.iloc[:, 1], label=f'{data.columns[1]} Forecast', color='orange')
...
def plot_impulse_response(var_model, steps, output_dir, title_prefix="Impulse Response"):
    ...
    plt.savefig(os.path.join(output_dir, f"{title_prefix.lower().replace(' ', '_')}_irf.png"))
    plt.close()
...
def run_analysis(input_file_arima_var, input_file_panel_var, output_dir, steps=10):
    ...
    arma_return = run_arima(df_arima_var['log_return'], df_arima_var[['daily_weighted_sentiment_lag1']], order=(0, 0, 0))
    arma_volume = run_arima(df_arima_var['log_volume'], df_arima_var[['daily_weighted_sentiment_lag3']], order=(0, 0, 2))
    ...
    plot_arima_fit(df_arima_var['log_return'], arma_return.fittedvalues, 'ARIMA Model Fit for Return', 'arima_return_fit.png', output_dir)
    plot_arima_fit(df_arima_var['log_volume'], arma_volume.fittedvalues, 'ARIMA Model Fit for Volume', 'arima_volume_fit.png', output_dir)
    ...
    save_variance_decomposition(var_return_model, steps, os.path.join(output_dir, "var_return_fevd.txt"))
    save_variance_decomposition(var_volume_model, steps, os.path.join(output_dir, "var_volume_fevd.txt"))
    plot_impulse_response(var_return_model, steps, output_dir, title_prefix="VAR Return Impulse Response")
    plot_impulse_response(var_volume_model, steps, output_dir, title_prefix="VAR Volume Impulse Response")
    ...
    df_panel_var = df_panel_var.dropna(subset=[f"sentiment_score_lag{lag}" for lag in range(1, 4)])
